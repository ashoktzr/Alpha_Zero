...
def run_clustering(X: pd.DataFrame, method: str='hdbscan', params: Dict=None) -> Tuple[pd.Series, pd.DataFrame]:
    ...
    Xnum = X.fillna(0).astype('float32')
    labels_arr = None
...
def compute_cluster_detailed_stats(df: pd.DataFrame, labels_col: str = 'cluster_id', dur_cap: Optional[int] = None) -> pd.DataFrame:
    ...
    # Ensure numeric types (float32 for memory efficiency where applicable)
    df['long_duration'] = pd.to_numeric(df['long_duration'], errors='coerce').astype('float32')
    df['short_duration'] = pd.to_numeric(df['short_duration'], errors='coerce').astype('float32')
    df['close'] = pd.to_numeric(df['close'], errors='coerce').astype('float32')
    df['exit_price'] = pd.to_numeric(df['exit_price'], errors='coerce').astype('float32')
